...
const { fakeProduct1, fakeReview } = require('./fake_products');
  ...
  describe('Product routes', () => {
    describe('GET routes', () => {
      ...
      it('Should fail when product dose not exist', async () => {
        await request(server)
          .get('/api/store/products/5bbe1ccf16be7b3cc40cb50c')
          .expect(404);
      });
    });
    ...
    describe('POST routes', () => {
      ...
      it('Should update quantity of a product', async () => {
        ...
        await request(server)
          .post('/api/store/products/quantity')
          .send({
      ...
      it('Should add a review', async () => {
        ...
        await request(server)
          .post('/api/store/products/reviews')
          .send({
            productId: product._id,
            review: fakeReview,
          })
